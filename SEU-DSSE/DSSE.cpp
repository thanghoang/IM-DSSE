		...
		MasterKey *pKey)
{
	...
	try
    {
        ...
        lstFree_column_idx.clear();
        lstFree_row_idx.clear();
        Miscellaneous::readHash_table(rT_W,gcsKwHashTable,gcsDataStructureFilepath,keywords_dictionary.size());
        Miscellaneous::readHash_table(rT_F,gcsFileHashTable,gcsDataStructureFilepath,rFileNames.size());
		...
		MasterKey *pKey) 
{
	...
	try
    {
        ...
        pKeywordCounterArray[pSearchToken.row_index] = pKeywordCounterArray[pSearchToken.row_index]+1;
    }
    catch(exception &e)
    {
                    ...
                    bool **pBlockStateMatrix)
{
	...
	try
    {
        ...
        for(index = 0, block_idx = 0; index < MAX_NUM_OF_FILES; index+=ENCRYPT_BLOCK_SIZE,block_idx++)
        {
            ...
            if(pSearchToken.hasRow_key == false || pBlockStateMatrix[row][block_idx]==ONE_VALUE)
            {
            ...
            pBlockStateMatrix[row][block_idx] = ZERO_VALUE;
 ...
 * @param global_counter: (input) global counter
    ...
    try
    {
        ...
        if(ENCRYPT_BLOCK_SIZE>1) //decrypt the received I' to update the new data representation I_bar to I'
        {
            ...
            for(I_bar_idx = 0 , idx = 0, size = MATRIX_ROW_SIZE*ENCRYPT_BLOCK_SIZE;idx < size; idx+=ENCRYPT_BLOCK_SIZE, I_bar_idx++)
            {
                ...
                memset(uchar_counter,0,BLOCK_CIPHER_SIZE);
                memcpy(&uchar_counter[BLOCK_CIPHER_SIZE/2],&global_counter,sizeof(TYPE_COUNTER));
                memcpy(&uchar_counter,&block_index,sizeof(TYPE_INDEX));
        ...
        /* Increase gc, set the new counter st to this block */
        
        pBlockCounterArray[block_index] = global_counter;
 ...
 * @param global_counter: (output) global_counter
 * @return	0 if successful
 */
            ...
            
{
    ...
    try
    {
        ...
        /* Increase the global counter to 1, set the counter of this block to gc, set the state of the block to 1.*/
        if(&global_counter != NULL)
            global_counter+=1;
        if(pBlockCounterArray!=NULL && &global_counter != NULL)
            pBlockCounterArray[block_idx] = global_counter;
        for(row = 0 ; row < MATRIX_ROW_SIZE; row++)
        {
            pBlockStateMatrix[row][block_idx] = ONE_VALUE;
        }
    }
    catch(exception & e)
    {
 ...
 *
 * @param new_adding_file_with_path: (input) full filename and path of the deleting file
...
/**
 * Function Name: fetchBlock_data
 *
 * Description:
 * get the block data from the encrypted data structure, given a block index
 *
 ...
 * @return	0 if successful
 */
int DSSE::fetchBlock_data(  TYPE_INDEX block_index,
                            MatrixType* I_prime,
                            MatrixType** I)
{
    
    TYPE_INDEX row, col, I_prime_idx;
    TYPE_INDEX begin,end;
    TYPE_INDEX I_bit_idx;
    TYPE_INDEX I_prime_col;
    TYPE_INDEX I_prime_bit_idx;
    ...
    try
    {
        ...
        if(ENCRYPT_BLOCK_SIZE > BYTE_SIZE)
        {
            begin = block_index * (ENCRYPT_BLOCK_SIZE/BYTE_SIZE);
        ...
        {
            col = (block_index*ENCRYPT_BLOCK_SIZE) / BYTE_SIZE;
            begin = (block_index*ENCRYPT_BLOCK_SIZE) % BYTE_SIZE;
    ...
    {
		cout << "Error occured in fetchBlock_data function " << e.what() << endl;
    ...
    for(int i = 0 ; i < n ; i++)
    {
        ...
        if(ENCRYPT_BLOCK_SIZE < BYTE_SIZE) // block size < byte size
        {
            for(row = 0 ; row < MATRIX_ROW_SIZE ; row++)
            {
                ...
                block_idx = (MATRIX_PIECE_COL_SIZE/ENCRYPT_BLOCK_SIZE)*i; //CHECK THIS!
        ...
        misc.write_matrix_to_file(filename,gcsMatrixPiecePath,I,MATRIX_ROW_SIZE,MATRIX_PIECE_COL_SIZE);
        ret = 0;
    }
exit:
    for(TYPE_INDEX m = 0 ; m < MATRIX_ROW_SIZE; m++)
...
/**
 * Function Name: precomputeRow_keys
 *
 * Description:
 ...
 * @return	0 if successful
 */
int DSSE::precomputeRow_keys(   TYPE_COUNTER* row_counter_arr,
...
int DSSE::loadEncrypted_matrix_from_files(MatrixType** I)
{
    ...
    for(TYPE_INDEX m = 0 ; m < MATRIX_ROW_SIZE; m++)
    {
        I_piece[m] = new MatrixType[MATRIX_PIECE_COL_SIZE];
    }
    for(int i = 0 ; i < n ; i++)
    {
        for(TYPE_INDEX m = 0 ; m < MATRIX_ROW_SIZE; m++)
        {
            memset(I_piece[m],0,MATRIX_PIECE_COL_SIZE);
        }
        string filename = std::to_string(i);
        misc.read_matrix_from_file(filename,gcsMatrixPiecePath,I_piece,MATRIX_ROW_SIZE,MATRIX_PIECE_COL_SIZE);
        for(col = 0; col < MATRIX_PIECE_COL_SIZE; col++)
        {
            I_col_idx = col+ (i*MATRIX_PIECE_COL_SIZE);
            for(row = 0 ; row < MATRIX_ROW_SIZE ; row++)
            {
                I[row][I_col_idx].byte_data = I_piece[row][col].byte_data;
            }
        }
    }
    for(TYPE_INDEX m = 0 ; m < MATRIX_ROW_SIZE; m++)
